        ...
        kernel = new ExecutionKernel({
            createCheckpoint: async (id) => {
                ...
                return {
                    ...
                    localStorage: rc.localStorage
                };
            },
            restoreCheckpoint: async (id, cp) => await runtime.restoreCheckpoint(id, cp)
        });
    });
    ...
    for (const fixture of fixtures) {
        test(`should successfully observe real-world fixture: ${fixture}`, async () => {
            ...
            const deterministicGraph = {
                ...
                nodes: baselineSnapshot.graph.nodes.filter((n) => n.type !== 'NetworkRequestNode'),
                edges: baselineSnapshot.graph.edges.filter((e) => e.type !== 'HAS_NETWORK_REQUEST')
            };
            ...
            if (fixture === 'anon-ecommerce-website') {
                ...
                const interactableNode = baselineSnapshot.graph.nodes.find((n) => n.type === 'DOMNode' && (n.properties.tagName === 'button' || n.properties.tagName === 'a'));
                if (interactableNode) {
